...
#### Scenario: Sophistry rebalances a corpus-skewed win-rate
- **WHEN** a corpus change has skewed the win-rate toward civilians and a later iteration equips the undercover with a steady-state disguise (one consistent false story across all rounds, eliminating the slip-rounds a cumulative-divergence civilian exploits) and civilians with a confidence threshold that only locks onto a clearly-standout player
- **THEN** the reported undercover win-rate rises back toward parity relative to the over-advantaged civilian iteration, the step is marked as swinging toward the undercover, and every iteration still reaches a terminal winner
