...
- 军备竞赛「胜率再均衡:卧底诡辩」(04 §7.5,回应用户「append 新词后平民/卧底胜率变得悬殊,需用诡辩等人类手法让胜率再次均衡」——2026-08-22):
  用户给 `words.ts` 追加 9 组高反差趣味词对后,卧底异词更易暴露 → 强平民档 `civ-refined`(跨轮累计)一度 **76.3% vs 23.8%** 大幅
  占优,胜率失衡。这正是「继续在 benchmark 上提升策略把胜率拉回」的契机。`arms-race-model.ts` 新增两个**人类高手手法**开关:
  ①`spyConsistent`(**稳态伪装**)——blend 决策去掉轮次盐 → 全程一致的假故事,消除 cumulative 平民赖以识别的「露馅轮」(一次编好、
  轮轮咬定,比临时圆谎更难被跨轮累计抓住);②`identifyGap`(平民**确信阈值**)——仅当最离群者比次离群者领先 ≥ 阈值才敢锁定,稳态诡辩
  把离群度差抹平到阈值之下 → 平民「看谁都差不多」、识别信号**结构性失灵**。两字段默认 `undefined` → 既有四档**逐字节不变**(向后兼容,
  有测试钉)。新增第五档 `spy-sophist`(spyBlend 0.95 + 两开关,identifyGap=0.06,plateau 0.04–0.10 across seeds 7/1/2/11 后选定):
  seed=7、80 局把 `civ-refined` 的 **76.3%/23.8%** 拉回 **50.0%/50.0%**(+26.3pp 归卧底),四步摆动 **civ↑→spy↑→civ↑→spy↑** 全部成立、
  **100% 完局**。证据 `docs/evidence/04-arms-race.md`(§5 新增诡辩档逐局复盘:卧底离群度 0.68–0.71 被压进平民簇 0.69–0.72、平民只在
  露头轮 `✓抓对`)。验收 `eval/arms-race.test.ts` 诡辩组(3):稳态伪装+确信阈值显著抬升卧底胜率 / 确信阈值是真实杠杆 / 新字段向后兼容
  逐字节等价。**诚实边界**:这是**为演示「失衡→再均衡」军备竞赛下一回合而设计**的确定性机制;胜率摆动仍是仿真耦合的产物,非人类逐局回放。
- 全局门禁再复核(2026-08-22 军备竞赛诡辩/再均衡层):`npm run build` EXIT 0 · `npm run test:node` **340 通过(45 文件;
