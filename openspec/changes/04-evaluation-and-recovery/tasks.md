...
> **本轮交付(2026-08-21)**:回答题面②真正问的问题——不是「描述有没有差异化」(那是 §5.3 compare 钉的描述质量),而是「学到更强策略后,**平民/卧底哪一方更容易赢**」。给出一条 civ↑→spy↑→civ↑→spy↑ 的军备竞赛曲线(四步、五档),并把技能档位标定在**可追溯的语料胜负信号**上。其中末段(7.5)演示「词库改动使胜率失衡后,用人类诡辩手法把它重新拉平」——军备竞赛可**收敛回均衡**而非单方失控。
...
- [x] 7.3 Report per-step civilian/undercover win-rate deltas with a swing-direction verdict, deterministic and complete. — `eval/arms-race.ts`:五档技能 `baseline→civ-awake→spy-counter→civ-refined→spy-sophist`,逐步胜率 diff + 摆动方向断言(期望 vs 实际)+ Markdown/JSONL 渲染;`tools/arms-race.ts`(`npm run arms-race:node`,`--games/--seed/--log/--report/--json`)摆动不成立或有未完局即 **exit 1**;落盘前 `scanSecrets` 双保险。实测(seed=7,80 局):平民胜率 **65%→85%→70%→76.3%→50%**,四步摆动 civ↑→spy↑→civ↑→spy↑ 全部成立、**100% 完局**。证据 `docs/evidence/04-arms-race.md`(+`.jsonl`);验收 `arms-race.test.ts`——复现/四步方向/非单调曲线(高—低—高—低)/因果/隔离脱敏。
- [x] 7.4 Emit a complete per-game, per-round trace so the win-rate is auditable ("how was it played out"), not just aggregate numbers. — `eval/arms-race-trace.ts` `extractGameTrace`/`renderGameTraceText`:每局落**描述→离群度→投票(带 `✓抓对`)→出局顺序→终局**全过程;`toArmsRaceTraceLines` + `tools/arms-race.ts --trace <path>` 落逐局逐轮脱敏 JSONL(五档全量,seed=7 共 **400 局**),`--sample N` 把每档「卧底被抓/逃脱」各一例打到 stdout。**三层日志均落 `docs/evidence/`**:①`--log` 聚合胜率 JSONL、②`--trace` 逐局逐轮 JSONL、③`--json-out` 完整报告 JSON(前两层嵌套成一棵树 + 原始 metrics,单文件全量存档)。Markdown 报告新增「§5 逐局复盘样本」内嵌三局机制示意。**脱敏**:秘密词一律不落,只以 `wordTag`(FNV 8-hex 假名)呈现——同词同 tag 即可分辨阵营而不泄词;role 终局后揭示(与生产终局同口径),`scanSecrets` 恒空。证:`arms-race.test.ts` trace 组(4)——结构完整/卧底第一轮多数最离群/JSONL 逐字节可复现+脱敏/可读复盘含抓对标记。

- [x] 7.5 Rebalance a corpus-skewed win-rate by advancing undercover strategy with a human sophistry tactic (steady-state disguise), proving the arms race can be re-levelled rather than run away. — 背景:词库(`words.ts`)追加 9 组高反差趣味词对后,卧底异词更易暴露 → 强平民档(`civ-refined`,跨轮累计)一度大幅占优(**~76% vs 24%**),胜率失衡。策略回应:`arms-race-model.ts` 新增两个人类高手手法开关——`spyConsistent`(**稳态伪装**:blend 决策去掉轮次盐 → 全程一致的假故事,消除 cumulative 平民赖以识别的「露馅轮」)+ `identifyGap`(平民**确信阈值**:仅当最离群者比次离群者领先 ≥ 阈值才敢锁定;稳态诡辩把离群度差抹平到阈值之下 → 平民「看谁都差不多」、识别信号结构性失灵)。两字段默认 `undefined` → 既有四档**逐字节不变**(向后兼容,`arms-race.test.ts` 断言)。新增第五档 `spy-sophist`(civSkill 1.0/cumulative,spyBlend 0.95,spyConsistent,identifyGap 0.06):实测(seed=7,80 局)把 `civ-refined` 的 **76.3%/23.8%** 拉回 **50.0%/50.0%**(+26.3pp 归卧底),四步摆动 **civ↑→spy↑→civ↑→spy↑** 全部成立、**100% 完局**。证据 `docs/evidence/04-arms-race.md`(§5 新增诡辩档逐局复盘:卧底离群度 0.68–0.71 被压进平民簇 0.69–0.72、平民只在露头轮才 `✓抓对`)。验收 `arms-race.test.ts` 诡辩组(3)——稳态伪装+确信阈值显著抬升卧底胜率 / 确信阈值是真实杠杆 / 新字段向后兼容逐字节等价——外加四步摆动组与非单调曲线(高—低—高—低)更新为五档。
